...
const DJView = () => {
  ...
  const handleDelete = async (recordId) => {
    try {
      ...
      // Crear la canción en la tabla de canciones eliminadas
      const newTableId = djId.replace(/^f|m$/g, ''); // Remover la 'f' inicial y 'm' final
      await createSongRequest(newTableId, songToDelete.fields['Song Name'], songToDelete.fields['Artist']);

      // Eliminar la canción de la tabla original
            ...
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Card sx={{ backgroundColor: '#fff', color: '#000', borderRadius:5 }}>
                <CardContent>
                  ...
                  <Box mt={2}>
                    <Button variant="contained" sx={{borderRadius:5}} color="error" fullWidth onClick={() => handleDelete(song.id)}>
                      Ya la puse
                    </Button>
                  </Box>
                </CardContent>
